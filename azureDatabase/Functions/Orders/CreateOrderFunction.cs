...
public class CreateOrderFunction
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        foreach (var request in requests)
        {
            ...
            // CastNames → JSON文字列に変換してSignalRメッセージへ設定
            string? castNameJson = request.CastNames.Count > 0
                ? System.Text.Json.JsonSerializer.Serialize(request.CastNames)
                : null;
